...
HCM EM cellmass, glucose and fermentation product trajectories, generated
using kinetic parameters reported by Kim et al. \cite{2008_kim_varner_ramkrishna_BiotechProg},
were compared with experimental measurements (Fig. \ref{fig:ecoli}A, dashed).
We estimated HCM FBA model parameters from the Kim et al. experimental data set using simulated annealing.
...
Taken together, these results suggested HCM FBA  was capable of describing experimental data, and is a viable reduced-order alternative to HCM EM.
...
HCM FBA captured the glucose to acetate metabolic shift for a large model of aerobic \textit{E. coli} metabolism (Fig. \ref{fig:ecoli}, right).
...
Elementary mode decomposition (and thus HCM EM) was not feasible for this network; 153,000 elementary modes were generated before the calculation prematurely terminated.
...
HCM FBA captured glucose consumption, cellmass formation and the dynamic switch between acetate production and consumption following glucose exhaustion.
Taken together, the aerobic \textit{E. coli} results demonstrated that HCM FBA described metabolic dynamics for large
networks that were infeasible for HCM EM. However, it was unclear whether all 29 FBA modes were required to capture the experimental data.
Next, we considered a systematic approach to identify the critical subset of FBA modes required for model performance.

Global variance based sensitivity analysis of aerobic \textit{E. coli} metabolism identified the FBA modes essential to biomass yield (Fig. \ref{fig:sensitivity}).
Total order sensitivity coefficients were calculated for all kinetic parameters and enzyme initial conditions.
Five of the 29 FBA modes were determined to be significant by a threshold test.
Removal of the most significant mode (encoding aerobic growth on glucose) destroyed model performance (Fig. \ref{fig:ecoli}B, dotted).
Conversely, removal of the remaining 24 modes had negligible effect on model performance (Fig. \ref{fig:ecoli}B, dashed).
Taken together, these results demonstrate that sensitivity analysis can identify critical FBA modes, thereby allowing further reduction to the minimal model structure
