...
HCM-EM has comparable internal flux estimates compared to metabolic flux analysis results \cite{2008_kim_varner_ramkrishna_BiotechProg}.
Next, we should compare the performance of HCM-FBA to lumped hybrid cybernetic models (L-HCM).
L-HCMs, which combine elementary modes into mode families based upon metabolic function \cite{2010_song,Song:2011aa},
has been applied to an \textit{E. coli} network with 67 reactions and a \textit{Saccharomyces cerevisiae} network (excluding compartmentalization) with 53 reactions;
both cases had satisfactory fits to extracellular experimental data.
However, while L-HCM reduces the dimension of modes participating in the optimal decision, it still requires the calculation of modes which for networks of even moderate size is not possible.
...
Thus, HCM-FBA opens up the possibility of manageable dynamic genome scale models of bacterial and perhaps even mammalian systems.
